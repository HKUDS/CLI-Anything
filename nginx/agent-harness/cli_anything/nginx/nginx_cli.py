...
from cli_anything.nginx.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary nginx command."""
    import subprocess
    result=subprocess.run(["nginx"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.nginx.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("nginx",version="1.0.0");skin.print_banner()
