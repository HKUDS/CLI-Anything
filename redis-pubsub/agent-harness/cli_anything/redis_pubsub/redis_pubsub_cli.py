...
from cli_anything.redis_pubsub.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary redis-pubsub command."""
    import subprocess
    result=subprocess.run(["redis-pubsub"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.redis_pubsub.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("redis-pubsub",version="1.0.0");skin.print_banner()
