...
from cli_anything.nikola.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary nikola command."""
    import subprocess
    result=subprocess.run(["nikola"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.nikola.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("nikola",version="1.0.0");skin.print_banner()
