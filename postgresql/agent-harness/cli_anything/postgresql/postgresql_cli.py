...
from cli_anything.postgresql.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary postgresql command."""
    import subprocess
    result=subprocess.run(["postgresql"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.postgresql.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("postgresql",version="1.0.0");skin.print_banner()
