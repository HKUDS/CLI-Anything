...
from cli_anything.memsql.core.session import Session
...
def run_cmd(args):
    ...
    import subprocess
    result=subprocess.run(["memsql"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.memsql.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("memsql",version="1.0.0");skin.print_banner()
