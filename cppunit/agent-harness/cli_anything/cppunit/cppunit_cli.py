...
from cli_anything.cppunit.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary cppunit command."""
    import subprocess
    result=subprocess.run(["cppunit"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.cppunit.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("cppunit",version="1.0.0");skin.print_banner()
