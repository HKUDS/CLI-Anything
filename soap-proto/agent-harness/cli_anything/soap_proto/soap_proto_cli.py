...
from cli_anything.soap_proto.core.session import Session
...
def run_cmd(args):
    """Run command."""
    import subprocess
    result=subprocess.run(["soap-proto"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.soap_proto.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("soap-proto",version="1.0.0");skin.print_banner()
