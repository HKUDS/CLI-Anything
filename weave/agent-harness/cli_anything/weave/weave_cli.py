...
from cli_anything.weave.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary weave command."""
    import subprocess
    result=subprocess.run(["weave"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.weave.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("weave",version="1.0.0");skin.print_banner()
