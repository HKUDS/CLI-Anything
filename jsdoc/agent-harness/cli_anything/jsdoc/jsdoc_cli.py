...
from cli_anything.jsdoc.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary jsdoc command."""
    import subprocess
    result=subprocess.run(["jsdoc"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.jsdoc.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("jsdoc",version="1.0.0");skin.print_banner()
