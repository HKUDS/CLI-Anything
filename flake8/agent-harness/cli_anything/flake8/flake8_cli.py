...
from cli_anything.flake8.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary flake8 command."""
    import subprocess
    result=subprocess.run(["flake8"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})

@cli.command()
@handle_error
def repl():
    from cli_anything.flake8.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("flake8",version="1.0.0");skin.print_banner()
