...
from cli_anything.openvino.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary benchmark_app command."""
    import subprocess
    result=subprocess.run(["benchmark_app"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.openvino.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("openvino",version="1.0.0");skin.print_banner()
