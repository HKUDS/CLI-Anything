...
from cli_anything.hapi_framework.core.session import Session
...
def run_cmd(args):
    """Run command."""
    import subprocess
    result=subprocess.run(["hapi-framework"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.hapi_framework.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("hapi-framework",version="1.0.0");skin.print_banner()
