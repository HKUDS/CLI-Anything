...
from cli_anything.adonis.core.session import Session
...
def run_cmd(args):
    ...
    import subprocess
    result=subprocess.run(["adonis"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.adonis.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("adonis",version="1.0.0");skin.print_banner()
