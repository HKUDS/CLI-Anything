...
from cli_anything.typescript.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary typescript command."""
    import subprocess
    result=subprocess.run(["typescript"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.typescript.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("typescript",version="1.0.0");skin.print_banner()
