...
from cli_anything.pigz.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary pigz command."""
    import subprocess
    result=subprocess.run(["pigz"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.pigz.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("pigz",version="1.0.0");skin.print_banner()
