...
from cli_anything.semistandard.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary semistandard command."""
    import subprocess
    result=subprocess.run(["semistandard"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.semistandard.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("semistandard",version="1.0.0");skin.print_banner()
