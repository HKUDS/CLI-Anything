...
from cli_anything.flow.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary flow command."""
    import subprocess
    result=subprocess.run(["flow"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.flow.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("flow",version="1.0.0");skin.print_banner()
