...
from cli_anything.lldb.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary lldb command."""
    import subprocess
    result=subprocess.run(["lldb"]+list(args),capture_output=True,text=True,timeout=300)
...
def repl():
    ...
    skin=ReplSkin("lldb",version="1.0.0");skin.print_banner()
