...
from cli_anything.mh.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary mh command."""
    import subprocess
    result=subprocess.run(["mh"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.mh.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("mh",version="1.0.0");skin.print_banner()
